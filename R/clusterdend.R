...
function(estimates_m=NULL, est_noctrls=NULL, picname, conditions=NULL, colors=NULL, 
    ...
    if(noctrls) {
        ...
        parameters <- setparam(labels)
        many.clusters_V(est_noctrls,resultsDir,paste(picname, "ClustersNoCtrls", sep="_"),"Clusters no ctrls", parameters=parameters, conditions=conditions, colors=colors)
    }
}
